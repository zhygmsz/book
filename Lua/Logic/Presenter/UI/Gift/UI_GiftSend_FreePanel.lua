module("UI_GiftSend_FreePanel",package.seeall);
...
local function UpdateGiftLimit()
    ...
    local valueLimit = GiftMgr.GetFreeCountLimit();
    local valueSent = GiftMgr.GetFreeCountSent();
...
local function OnSystemClose()
    mSelectedGifts = {};
    UIMgr.UnShowUI(AllUI.UI_GiftSend_FreePanel);
end
...
function SetSelectedGift(gift)
    local select = mSelectedGifts[gift]  or 0;
    select = select + 1;
    ...
    local countLimit = GiftMgr.GetFreeCountLimit();
    local countSent = GiftMgr.GetFreeCountSent();
    if total + countSent +1 > countLimit then
        TipsMgr.TipByKey("gift_count_limit_notice",countLimit - countLimit );
        return;
    end
    mSelectedGifts[gift] = select;
    return true;
end
...
function OnCreate(ui)
    ...
    mData._friendTable = BaseWrapContentEx.new(ui,path.."/FriendWidget/FriendContent/Scroll View",8,UIGiftFriendWrapUIEx,nil,UI_GiftSend_FreePanel);
    ...
    local giftInfoTable = {gifts = gifts,callbacks = {OnItemAddClick,OnItemDeleteClick},context = UI_GiftSend_FreePanel};

    mData._giftBagCom = GiftComponentBag.new(ui,giftInfoTable);

    local input = ui:FindComponent("LuaUIInput","Offset/ChatWidget/Input");
    local countLimit = ConfigData.GetIntValue("Gift_bless_input_limit");
