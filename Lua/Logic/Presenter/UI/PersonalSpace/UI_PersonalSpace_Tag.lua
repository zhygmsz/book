module("UI_PersonalSpace_Tag",package.seeall)
...
function PlayerInfoLoaded(playerid,playerInfo)
    if tostring(playerid) == tostring(UserData.PlayerID) then
        ...
        if mPlayerInfo then
            ...
            local othertags = mPlayerInfo:GetCharacterTagsFromOther()
            local showsystags = mPlayerInfo:GetCharacterTagsFromSystemShowing()
            local systags = mPlayerInfo:GetCharacterTagsFromSystem()
            m_userTags[1] = tags
            m_userTags[2] = othertags
            m_userTags[3] = showsystags
            mSystemTags.show=showsystags
            mSystemTags.tags = systags
            if mEditorSysTag then
                ...
                for i=1,nn do
                    temp[i] = mSystemTags.show[i]
                    check[mSystemTags.show[i]]=1
                end
                local index = 1
                for i=1,#mSystemTags.tags do
                    local tagid = mSystemTags.tags[i]
                    if check[tagid]==nil then
...
function TagChanged(playerid,tags)
    mPlayerId=playerid
    GetData()
end
        ...
        for i=1,max do
            ...
            if i<=datanum then
               ...
               -- item.popEvent.id =100000+ 10000*j+1000 + i
                item.dataIndex = i
                if j==1 or j==2 then
                    local tag = CharacterTagData.GetCharacterTag(m_users[i])
                    item.itemlabel.text = tag.value
                elseif j==3 and mEditorSysTag then
